...
const links = [
  ...
  { to: "/developer", label: "Developer", icon: "🛠", color: "oklch(0.68 0.22 200)" },
] as const;
  ...
  return (
    ...
    >
          ...
          {links.map((l) => {
            ...
            return (
              <Link
                ...
                style={{
                  color: active ? "oklch(0.95 0.01 255)" : "oklch(0.58 0.04 255)",
                  background: active ? "oklch(1 0 0 / 8%)" : "transparent",
                }}
                onMouseEnter={(e) => {
                  if (!active)
                    (e.currentTarget as HTMLElement).style.color = "oklch(0.82 0.01 255)";
                }}
                onMouseLeave={(e) => {
                  if (!active)
                    (e.currentTarget as HTMLElement).style.color = "oklch(0.58 0.04 255)";
                }}
              >
                {active && (
                  <span
      ...
      <div
        ...
        style={{ maxHeight: open ? "700px" : "0", opacity: open ? 1 : 0, overflow: "hidden" }}
      >
        <div
          className="px-4 pb-4 pt-1 grid grid-cols-2 gap-1"
          ...
          {links.map((l) => {
            ...
            return (
              <Link
                ...
                style={{
                  background: active ? "oklch(1 0 0 / 7%)" : "transparent",
                  color: active ? "oklch(0.95 0.01 255)" : "oklch(0.62 0.04 255)",
