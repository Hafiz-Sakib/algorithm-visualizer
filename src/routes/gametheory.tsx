...
/* ------------------------------------------------------------------ */
/* Step-player hook — returns the SAME prop names Controls expects     */
/* (onPlay/onPause/onReset/onStepBack/onStepFwd) so {...p} just works. */
/* ------------------------------------------------------------------ */
...
function buildNash(M: Cell[][]): NashStep[] {
  ...
  for (let row = 0; row < 2; row++) {
    const left = M[row][0].a;
    const right = M[row][1].a;
    const best = left >= right ? 0 : 1;
    bestA[row][best] = true;
    if (left === right) {
      bestA[row][0] = true;
      bestA[row][1] = true;
  ...
  for (let r = 0; r < 2; r++)
    for (let c = 0; c < 2; c++)
      if (bestA[r][c] && bestB[r][c]) {
        eq.push([r, c]);
        steps.push({ scan: [r, c], bestA: clone(bestA), bestB: clone(bestB), equilibria: [...eq], note: `Mutual best response → Nash equilibrium at (${r},${c}).` });
      }
...
function NashViz({ speed, accent }: { speed: number; accent: string }) {
  ...
  const s = p.current;
  const rowLabels = ["A: Silent", "A: Confess"];
  const colLabels = ["B: Silent", "B: Confess"];
            ...
            {[0, 1].map((r) => (
              <Row key={r}>
                ...
                {[0, 1].map((c) => {
                  ...
                  const ba = s?.bestA?.[r]?.[c];
                  const bb = s?.bestB?.[r]?.[c];
                  return (
                    <motion.div key={c} animate={{ scale: scanning ? 1.05 : 1 }} className="relative rounded-xl p-4 sm:p-6 min-w-[120px]"
                      style={{
                        background: isEq ? `${accent}26` : scanning ? "oklch(1 0 0 / 8%)" : "oklch(1 0 0 / 4%)",
                        border: `2px solid ${isEq ? accent : scanning ? "oklch(1 0 0 / 25%)" : "oklch(1 0 0 / 8%)"}`,
                        boxShadow: isEq ? `0 0 18px ${accent}55` : "none",
                      }}>
                      <div className="flex justify-between text-lg font-mono font-bold">
                        <span style={{ color: ba ? "var(--accent)" : "oklch(0.5 0.05 255)" }}>{M[r][c].a}</span>
                        <span style={{ color: bb ? "var(--warn)" : "oklch(0.5 0.05 255)" }}>{M[r][c].b}</span>
                      </div>
                      {isEq && (
                        <span className="absolute -top-2 -right-2 text-[9px] font-bold px-1.5 py-0.5 rounded-full" style={{ background: accent, color: "oklch(0.08 0.02 265)" }}>NE</span>
                      )}
                    </motion.div>
                  );
                })}
...
function buildTree() {
  ...
  let id = 0;
  const make = (depth: number, x: number): number => {
    const myId = id++;
    nodes[myId] = { id: myId, depth, x, children: [] };
    return myId;
  };
  const r = make(0, 4);
  ...
  lvl1.forEach((pNode, pi) => {
    ...
    lvl2.forEach((q, qi) => {
      ...
      lvl3.forEach((leaf) => {
        nodes[leaf].leaf = leafVals[li++];
        nodes[leaf].value = nodes[leaf].leaf;
        leaves.push(leaf);
      });
    });
  });
  return { nodes, root: r, leaves };
}
...
function buildMinimax() {
  ...
  Object.values(nodes).forEach((n) => {
    if (n.leaf !== undefined) values[n.id] = n.leaf;
  });
  const mm = (id: number, maxing: boolean): number => {
    const n = nodes[id];
    if (n.leaf !== undefined) {
      steps.push({ visiting: id, values: { ...values }, note: `Leaf value ${n.leaf}.` });
      return n.leaf;
    }
      ...
      <VizFrame>
        <svg viewBox={`0 0 ${W} ${H}`} className="w-full" style={{ height: 340 }}>
          {Object.values(tree.nodes).map((n) =>
            n.children.map((c) => {
              const a = pos(n), b = pos(tree.nodes[c]);
              return <line key={`${n.id}-${c}`} x1={a.x} y1={a.y} x2={b.x} y2={b.y} stroke="oklch(1 0 0 / 12%)" strokeWidth={1.5} />;
            }),
          )}
          {Object.values(tree.nodes).map((n) => {
            const { x, y } = pos(n);
            const visiting = s?.visiting === n.id;
            const val = s?.values?.[n.id];
            const isLeaf = n.leaf !== undefined;
            const maxing = n.depth % 2 === 0;
            return (
              <g key={n.id}>
                <motion.circle cx={x} cy={y} animate={{ r: visiting ? 19 : 15 }}
                  fill={visiting ? accent : isLeaf ? "oklch(0.30 0.06 265)" : maxing ? "oklch(0.30 0.10 255)" : "oklch(0.30 0.10 22)"}
                  stroke={visiting ? "#fff" : maxing ? "var(--accent)" : "var(--danger)"} strokeWidth={visiting ? 2.5 : 1.5}
                  style={{ filter: visiting ? `drop-shadow(0 0 8px ${accent})` : "none" }} />
  ...
  for (let i = 0; i < n; i++)
    for (let j = i + 1; j < n; j++) {
      ...
      for (let r = 0; r < rounds; r++) {
        const mi = STRATS[i].fn(oi, oj);
        const mj = STRATS[j].fn(oj, oi);
        oi.push(mi); oj.push(mj);
        scores[i] += pay(mi, mj);
        scores[j] += pay(mj, mi);
    ...
    <div className="space-y-4">
      <VizFrame>
        <div className="space-y-2.5 py-4 px-2">
          {STRATS.map((st, i) => {
            const active = s?.matchA === i || s?.matchB === i;
            ...
            return (
              <div key={st.name} className="flex items-center gap-3">
                ...
                <div className="flex-1 h-6 rounded-md overflow-hidden" style={{ background: "oklch(1 0 0 / 5%)" }}>
                  <motion.div className="h-full rounded-md flex items-center justify-end pr-2" animate={{ width: `${(score / max) * 100}%` }} transition={{ type: "spring", stiffness: 200, damping: 26 }}
                    style={{ background: active ? `linear-gradient(90deg, ${accent}, oklch(0.75 0.18 162))` : "linear-gradient(90deg, oklch(0.45 0.10 265), oklch(0.35 0.08 265))", boxShadow: active ? `0 0 12px ${accent}66` : "none" }}>
          ...
          <div className="flex justify-center gap-6 pb-4">
            {[s.matchA, s.matchB].map((idx, k) => (
              <div key={k} className="flex items-center gap-2 text-xs">
                <span style={{ color: "oklch(0.6 0.04 255)" }}>{STRATS[idx]?.name}</span>
                <span className="font-mono font-bold px-2 py-0.5 rounded" style={{ background: s.lastMoves![k] === "C" ? "var(--accent)" : "var(--danger)", color: "oklch(0.08 0.02 265)" }}>
                  {s.lastMoves![k] === "C" ? "Cooperate" : "Defect"}
                </span>
              </div>
            ))}
          </div>
...
function buildFP(iters = 24): FpStep[] {
  ...
  const countsA: [number, number] = [1, 0];
  const countsB: [number, number] = [0, 1];
  const steps: FpStep[] = [];
  for (let t = 0; t < iters; t++) {
    const freqB = countsB[0] / (countsB[0] + countsB[1]);
    const eaIf0 = A[0][0] * freqB + A[0][1] * (1 - freqB);
    const eaIf1 = A[1][0] * freqB + A[1][1] * (1 - freqB);
    const moveA = eaIf0 >= eaIf1 ? 0 : 1;
    const freqA = countsA[0] / (countsA[0] + countsA[1]);
    const ebIf0 = A[0][0] * freqA + A[1][0] * (1 - freqA);
    const ebIf1 = A[0][1] * freqA + A[1][1] * (1 - freqA);
    const moveB = ebIf0 <= ebIf1 ? 0 : 1;
    ...
    <div className="space-y-4">
      <VizFrame>
        <svg viewBox={`0 0 ${W} ${H}`} className="w-full" style={{ height: 250 }}>
          ...
          {history.length > 0 && (
            <>
              <circle cx={((history.length - 1) / Math.max(1, steps.length - 1)) * W} cy={H - (s?.freqA ?? 0) * H} r={5} fill={accent} style={{ filter: `drop-shadow(0 0 6px ${accent})` }} />
              <circle cx={((history.length - 1) / Math.max(1, steps.length - 1)) * W} cy={H - (s?.freqB ?? 0) * H} r={5} fill="var(--warn)" />
            </>
          )}
        </svg>
        <div className="flex justify-center gap-8 pb-3 text-xs font-mono">
          <span style={{ color: accent }}>P(A plays Top) = {(s?.freqA ?? 0).toFixed(2)}</span>
          <span style={{ color: "var(--warn)" }}>P(B plays Left) = {(s?.freqB ?? 0).toFixed(2)}</span>
        </div>
      </VizFrame>
      <NoteBar note={s?.note} accent={accent} />
...
function buildTFT(rounds = 12): TftStep[] {
  ...
  for (let r = 0; r < rounds; r++) {
    const myMove: Move = r === 0 ? "C" : opp[r - 1];
    const oppMove = oppScript[r % oppScript.length];
    me.push(myMove); opp.push(oppMove);
    sMe += pay(myMove, oppMove);
    sOpp += pay(oppMove, myMove);
          ...
          {rows.map(([label, moves]) => (
            <div key={label} className="flex items-center gap-3">
              ...
              <div className="flex gap-1.5 flex-wrap">
                <AnimatePresence>
                  {moves.map((m, i) => (
                    <motion.div key={i} initial={{ scale: 0, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="h-8 w-8 rounded-lg grid place-items-center text-xs font-mono font-bold" style={{ background: m === "C" ? "var(--accent)" : "var(--danger)", color: "oklch(0.08 0.02 265)" }}>{m}</motion.div>
                  ))}
                </AnimatePresence>
              </div>
            </div>
          ))}
          <div className="flex gap-6 pt-2 text-sm font-mono">
            <span style={{ color: accent }}>TFT: {s?.scoreMe ?? 0}</span>
            <span style={{ color: "var(--warn)" }}>Opp: {s?.scoreOpp ?? 0}</span>
          </div>
        </div>
      </VizFrame>
      <NoteBar note={s?.note} accent={accent} />
...
function buildElFarol(agents = 60, weeks = 30, cap = 0.6): ElStep[] {
  ...
  for (let w = 0; w < weeks; w++) {
    ...
    for (let a = 0; a < agents; a++) {
      const predict = Math.min(agents, last * weight[a]);
      going.push(predict < threshold);
    }
...
function ElFarolViz({ speed, accent }: { speed: number; accent: string }) {
  const AGENTS = 60;
  const THRESH = Math.floor(AGENTS * 0.6);
      ...
      <VizFrame>
        <div className="grid grid-cols-12 gap-1.5 p-4 place-items-center">
          {Array.from({ length: AGENTS }).map((_, i) => {
            const going = s?.going?.[i];
            return <motion.div key={i} animate={{ scale: going ? 1 : 0.7 }} className="h-3.5 w-3.5 rounded-full" style={{ background: going ? accent : "oklch(0.30 0.04 265)", boxShadow: going ? `0 0 6px ${accent}aa` : "none" }} />;
          })}
        </div>
        ...
        <div className="flex justify-center gap-6 pb-3 text-xs font-mono">
          <span style={{ color: accent }}>Attendance: {s?.attendance ?? 0}/{AGENTS}</span>
          <span style={{ color: "var(--warn)" }}>Threshold: {THRESH}</span>
        </div>
      </VizFrame>
      <NoteBar note={s?.note} accent={accent} />
  ...
  return (
    <div className="rounded-xl px-4 py-2.5 text-sm" style={{ background: `${accent}14`, border: `1px solid ${accent}33`, color: "oklch(0.85 0.02 255)" }}>
      <AnimatePresence mode="wait">
        <motion.span key={note} initial={{ opacity: 0, y: 4 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -4 }} transition={{ duration: 0.18 }}>{note ?? ""}</motion.span>
      </AnimatePresence>
    </div>
  );
}
...
function Legend({ items }: { items: { color: string; label: string }[] }) {
  return (
    <div className="flex flex-wrap gap-3 text-[11px]" style={{ color: "oklch(0.50 0.04 255)" }}>
      {items.map((l) => (
        <span key={l.label} className="flex items-center gap-1.5"><span className="h-2 w-2 rounded-sm" style={{ background: l.color }} />{l.label}</span>
      ))}
    </div>
  );
}
...
const EXPLAIN: Record<Algo, { idea: string; how: string[]; real: string }> = {
  "Nash Equilibrium": {
    idea: "A cell is a Nash equilibrium when neither player can gain by unilaterally switching. We find it by marking each player's best response, then looking for cells both marked.",
    how: [
      "For each of B's choices, mark A's payoff-maximizing row.",
      "For each of A's choices, mark B's payoff-maximizing column.",
      "Any cell marked by BOTH is a pure-strategy Nash equilibrium.",
      "In the Prisoner's Dilemma the only NE is (Confess, Confess).",
    ],
    real: "Pricing wars: two shops each cut prices because undercutting is each one's best response — even though both staying high would be better.",
  },
  Minimax: {
    idea: "In a zero-sum game one player maximizes the score while the other minimizes it. Minimax backs values up a game tree: MAX layers take the max of children, MIN layers take the min.",
    how: [
      "Recurse to the leaves (final game outcomes).",
      "At a MAX node return the largest child value.",
      "At a MIN node return the smallest child value.",
      "The root value is the game's outcome under perfect play.",
    ],
    real: "Chess / tic-tac-toe engines: assume the opponent plays optimally and pick the move with the best worst-case result.",
  },
  "Iterated Dilemma": {
    idea: "Strategies meet repeatedly in the Prisoner's Dilemma. A round-robin tournament reveals which long-run strategy accumulates the most points (Axelrod's famous experiment).",
    how: [
      "Each pair of strategies plays many rounds.",
      "Each round, both pick Cooperate or Defect from history.",
      "Payoffs: T=5 > R=3 > P=1 > S=0.",
      "Sum scores across all matchups; highest total wins.",
    ],
    real: "Cooperation in nature, trade agreements, and online reputation systems where repeated interaction rewards being 'nice but retaliatory'.",
  },
  "Fictitious Play": {
    idea: "A learning dynamic: each player assumes the opponent plays according to the historical frequency of past moves, and best-responds to that belief. Often converges to an equilibrium.",
    how: [
      "Track how often each opponent action has occurred.",
      "Estimate opponent's mixed strategy from those counts.",
      "Play the best response to that estimate.",
      "Update counts; over time frequencies converge.",
    ],
    real: "Markets and adaptive agents that learn rivals' tendencies from observed behavior rather than knowing them in advance.",
  },
  "Tit-for-Tat": {
    idea: "A single repeated match. Tit-for-Tat cooperates on round one, then simply copies whatever the opponent did last round — nice, retaliatory, forgiving, and clear.",
    how: [
      "Round 1: cooperate.",
      "Round n: replay opponent's move from round n-1.",
      "Punishes a defection immediately, once.",
      "Forgives as soon as the opponent returns to cooperating.",
    ],
    real: "Trust-building in negotiations and arms control: mirror good faith, answer betrayal proportionally, then reset.",
  },
  "El Farol Bar": {
    idea: "A bounded-rationality / congestion game. Many agents independently decide whether to go to a bar that's only fun if under-crowded. No single rule works for everyone, yet attendance self-organizes.",
    how: [
      "Each agent predicts attendance from recent weeks.",
      "Go only if predicted crowd is below the comfort threshold.",
      "Actual attendance feeds back into next week's predictions.",
      "The crowd hovers around the threshold without coordination.",
    ],
    real: "Traffic routing, network congestion, and choosing off-peak times — decentralized agents balancing a shared resource.",
  },
};
...
def nash_equilibria(A, B):          # A,B = payoff matrices
    ...
    # A's best response to each of B's columns
    for c in range(cols):
    ...
    # B's best response to each of A's rows
    for r in range(rows):
  ...
  Minimax: `# Minimax value of a zero-sum game tree
...
# Optional alpha-beta pruning for speed
...
def play(stratA, stratB, rounds=200):
    a_hist, b_hist, sa, sb = [], [], 0, 0
    for _ in range(rounds):
        ma = stratA(a_hist, b_hist)
        mb = stratB(b_hist, a_hist)
        pa, pb = PAYOFF[(ma, mb)]
        sa += pa; sb += pb
        a_hist.append(ma); b_hist.append(mb)
    return sa, sb
...
def tournament(strategies):
    ...
    for i in range(len(names)):
        for j in range(i+1, len(names)):
            ...
            scores[names[j]] += sb
    return scores`,
  "Fictitious Play": `# Fictitious play: best-respond to opponent's empirical frequency
def fictitious_play(A, iters=1000):
    # A[i][j] = row player's payoff; column player minimizes it
    n, m = len(A), len(A[0])
    countA = [1] + [0]*(n-1)      # belief about row plays
    countB = [1] + [0]*(m-1)      # belief about col plays

    for _ in range(iters):
        # row best-responds to belief over columns
        freqB = [c/sum(countB) for c in countB]
        eu = [sum(A[i][j]*freqB[j] for j in range(m)) for i in range(n)]
        i = max(range(n), key=lambda i: eu[i])

        # column best-responds (minimizes) to belief over rows
        freqA = [c/sum(countA) for c in countA]
        ec = [sum(A[i][j]*freqA[i] for i in range(n)) for j in range(m)]
...
def tit_for_tat(my_history, opp_history):
    ...
    return opp_history[-1]     # then copy opponent's last move
...
def generous_tft(my, opp, forgive=0.1):
    ...
    if opp[-1] == "D" and random.random() < forgive:
        return "C"             # occasionally forgive a defection
    return opp[-1]`,
...
def el_farol(agents=100, weeks=100, capacity=0.6):
    ...
    for _ in range(weeks):
        ...
        for a in range(agents):
            predicted = min(agents, last * weight[a])
            if predicted < threshold:   # expect it won't be crowded
                going += 1
        history.append(going)
    return history   # attendance oscillates around the threshold`,
};
    ...
    <div className="rounded-2xl overflow-hidden" style={panel}>
      ...
      <div className="flex items-center gap-1 px-3 pt-3">
        {(["explain", "code"] as const).map((t) => (
          <button key={t} onClick={() => setTab(t)} className="px-3 py-1.5 rounded-lg text-xs font-medium transition-all"
            style={{
              background: tab === t ? accent : "oklch(1 0 0 / 6%)",
              color: tab === t ? "oklch(0.08 0.02 265)" : "oklch(0.65 0.04 255)",
              border: `1px solid ${tab === t ? accent : "oklch(1 0 0 / 10%)"}`,
            }}>
            {t === "explain" ? "Explanation" : "Python code"}
          </button>
        ))}
        <span className="ml-auto flex gap-2 text-[10px] font-mono" style={{ color: "oklch(0.5 0.04 255)" }}>
          <span>⏱ {cx.time}</span>
          <span>▢ {cx.space}</span>
        </span>
      </div>
      ...
      <div className="p-4">
        {tab === "explain" ? (
          <div className="space-y-3">
            ...
            <div>
              ...
              <ol className="space-y-1.5">
                {ex.how.map((h, i) => (
                  <li key={i} className="flex gap-2.5 text-sm" style={{ color: "oklch(0.72 0.03 255)" }}>
                    <span className="shrink-0 h-5 w-5 rounded-md grid place-items-center text-[10px] font-bold" style={{ background: `${accent}22`, color: accent }}>{i + 1}</span>
                    <span>{h}</span>
                  </li>
                ))}
              </ol>
          ...
          <pre className="overflow-x-auto rounded-xl p-3.5 text-[12px] leading-relaxed font-mono"
            style={{ background: "oklch(0.07 0.015 265)", border: "1px solid oklch(1 0 0 / 8%)", color: "oklch(0.82 0.03 255)" }}>
            <code>{CODE[algo]}</code>
          </pre>
        )}
      </div>
      ...
      <header className="flex flex-wrap items-start justify-between gap-4">
        <div>
          <div className="flex items-center gap-2 mb-1">
            ...
            <h1 className="text-xl sm:text-2xl font-bold tracking-tight" style={{ letterSpacing: "-0.025em" }}>Game Theory</h1>
          </div>
          <p className="text-sm" style={{ color: "oklch(0.55 0.04 255)" }}>{DESC[algo]}</p>
        </div>
        <div className="flex flex-wrap gap-1.5">
          {ALGOS.map((name) => (
            <button key={name} onClick={() => setAlgo(name)} className="px-3 py-1.5 rounded-lg text-xs font-medium transition-all duration-150 hover:scale-105"
              style={{
                background: algo === name ? COLOR[name] : "oklch(1 0 0 / 6%)",
                color: algo === name ? "oklch(0.08 0.02 265)" : "oklch(0.65 0.04 255)",
                border: `1px solid ${algo === name ? COLOR[name] : "oklch(1 0 0 / 10%)"}`,
                boxShadow: algo === name ? `0 0 12px ${COLOR[name]}40` : "none",
              }}>
              {name}
            </button>
          ))}
