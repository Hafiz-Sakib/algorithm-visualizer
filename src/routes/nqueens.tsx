...
export const Route = createFileRoute("/nqueens")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Backtracking N-Queens solver visualization with C++ source." },
    ],
  }),
        ...
        <header className="flex flex-wrap items-start justify-between gap-4">
          <div>
            <div className="flex items-center gap-2 mb-1">
              ...
              <h1 className="text-xl sm:text-2xl font-bold tracking-tight" style={{ letterSpacing: "-0.025em" }}>N-Queens</h1>
            </div>
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            >
              {[4, 5, 6, 7, 8].map((v) => <option key={v} value={v} style={{ background: "oklch(0.14 0.02 265)", color: "oklch(0.92 0.01 255)" }}>{v}×{v}</option>)}
            </select>
          </div>
        </header>
