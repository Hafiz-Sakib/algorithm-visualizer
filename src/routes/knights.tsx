...
export const Route = createFileRoute("/knights")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Chess-board Knight's Tour visualization using Warnsdorff's heuristic, with C++ source." },
    ],
  }),
  ...
  return (
    <div className="space-y-4 py-2">
      <div className="space-y-4 min-w-0">
        <header className="flex flex-wrap items-start justify-between gap-4">
          <div>
            <div className="flex items-center gap-2 mb-1">
              ...
              <h1 className="text-xl sm:text-2xl font-bold tracking-tight" style={{ letterSpacing: "-0.025em" }}>Knight's Tour</h1>
            </div>
          ...
          <div className="flex flex-wrap items-center gap-2">
            <label className="text-[11px] uppercase tracking-wider" style={{ color: "oklch(0.50 0.04 255)" }}>Board</label>
            <select
              value={n}
        ...
        {/* Board */}
        <div className="rounded-2xl overflow-hidden p-4 flex justify-center" style={{ background: "oklch(0.10 0.02 265)", border: "1px solid oklch(1 0 0 / 8%)" }}>
        ...
        <div className="flex flex-wrap gap-3 text-[11px]" style={{ color: "oklch(0.50 0.04 255)" }}>
          <span className="flex items-center gap-1.5"><span className="h-2 w-2 rounded-sm" style={{ background: ACCENT }} />Knight</span>
          <span className="flex items-center gap-1.5"><span className="h-2 w-2 rounded-sm" style={{ background: "oklch(0.45 0.16 220 / 90%)" }} />Visited (number = move order)</span>
          <span className="ml-auto font-mono">Step <span style={{ color: ACCENT }}>{current?.step ?? 0}</span> / {n * n}</span>
        </div>
