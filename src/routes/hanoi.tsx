...
const PEG_COLORS = [
  "oklch(0.72 0.19 255)", "oklch(0.75 0.18 162)", "oklch(0.82 0.18 85)",
  "oklch(0.68 0.22 22)", "oklch(0.75 0.18 310)", "oklch(0.72 0.22 180)",
  "oklch(0.80 0.18 45)", "oklch(0.73 0.20 140)",
];
  ...
  return (
    <div className="space-y-4 py-2">
      <div className="space-y-4 min-w-0">
        <header className="flex flex-wrap items-start justify-between gap-4">
          <div>
            <div className="flex items-center gap-2 mb-1">
              ...
              <h1 className="text-xl sm:text-2xl font-bold tracking-tight" style={{ letterSpacing: "-0.025em" }}>Tower of Hanoi</h1>
            </div>
            <p className="text-sm" style={{ color: "oklch(0.55 0.04 255)" }}>
              Move all {n} disks from A to C — optimal solution uses 2<sup>{n}</sup>−1 = {(1 << n) - 1} moves.
            </p>
          </div>
          <div className="flex flex-wrap items-center gap-2">
            <label className="text-[11px] uppercase tracking-wider" style={{ color: "oklch(0.50 0.04 255)" }}>Disks</label>
            <select
              value={n}
        ...
        {/* Towers */}
        <div className="rounded-2xl overflow-hidden p-4" style={{ background: "oklch(0.10 0.02 265)", border: "1px solid oklch(1 0 0 / 8%)" }}>
          <div className="grid grid-cols-3 gap-2 sm:gap-6 items-end h-[260px] sm:h-[300px]">
            {pegs.map((peg, pi) => (
              <div key={pi} className="relative flex flex-col items-center justify-end h-full">
                {/* Peg rod */}
                <div className="absolute bottom-0 left-1/2 -translate-x-1/2" style={{ width: 4, height: "85%", background: "oklch(0.30 0.03 265)", borderRadius: 2 }} />
                {/* Base */}
                <div className="absolute bottom-0 left-1/2 -translate-x-1/2" style={{ width: "92%", maxWidth: maxDiskWidth + 40, height: 6, background: "oklch(0.35 0.03 265)", borderRadius: 3 }} />
                {/* Disks */}
                <div className="relative flex flex-col-reverse items-center w-full" style={{ paddingBottom: 6 }}>
                  <AnimatePresence>
                    {peg.map((disk, di) => {
                      const w = unit * disk + 20;
                ...
                <span className="mt-2 text-[11px] font-mono uppercase tracking-widest" style={{ color: "oklch(0.50 0.04 255)" }}>
        ...
        <div className="flex flex-wrap gap-3 text-[11px] items-center" style={{ color: "oklch(0.50 0.04 255)" }}>
          <span className="font-mono">Moves: <span style={{ color: ACCENT }}>{current?.count ?? 0}</span> / {current?.total ?? (1 << n) - 1}</span>
        </div>

        <Controls
          playing={playing} onPlay={play} onPause={pause} onReset={reset}
